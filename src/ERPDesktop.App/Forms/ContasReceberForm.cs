...
{
    private readonly ITituloReceberRepository _repo;
    private readonly ClienteAppService _clientes;

    public ContasReceberForm(ITituloReceberRepository repo, ClienteAppService clientes)
    {
        _repo = repo;
        _clientes = clientes;
        InitializeComponent();
        ...
        _ts.Items.Add(Mk("Receber", BtnReceber_Click));
    ...
    private ToolStripButton Mk(string t, EventHandler c)
    {
        var b = new ToolStripButton(t, null, c) { DisplayStyle = ToolStripItemDisplayStyle.Text };
        return b;
    }
    ...
    {
        _grid.Columns.Clear();
        _grid.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "Cliente", DataPropertyName = nameof(TituloReceberGridRow.ClienteNome), FillWeight = 28 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "Descrição", DataPropertyName = nameof(TituloReceberGridRow.Descricao), FillWeight = 32 });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "Vencimento", DataPropertyName = nameof(TituloReceberGridRow.Vencimento), FillWeight = 14, DefaultCellStyle = new DataGridViewCellStyle { Format = "dd/MM/yyyy" } });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "Valor", DataPropertyName = nameof(TituloReceberGridRow.Valor), FillWeight = 12, DefaultCellStyle = new DataGridViewCellStyle { Format = "C2", Alignment = DataGridViewContentAlignment.MiddleRight } });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "Recebido", DataPropertyName = nameof(TituloReceberGridRow.ValorRecebido), FillWeight = 12, DefaultCellStyle = new DataGridViewCellStyle { Format = "C2", Alignment = DataGridViewContentAlignment.MiddleRight } });
        _grid.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = "Situação", DataPropertyName = nameof(TituloReceberGridRow.Situacao), FillWeight = 12 });
    ...
    private long? IdSel() =>
        _grid.CurrentRow?.DataBoundItem is TituloReceberGridRow r ? r.Id : null;

    private void BtnNovo_Click(object? s, EventArgs e)
    {
        using var dlg = new Form
        {
            Text = "Novo título a receber",
        ...
        cb.DisplayMember = nameof(ClienteGridRow.NomeRazaoSocial);
        cb.ValueMember = nameof(ClienteGridRow.Id);
        cb.DataSource = _clientes.PesquisarParaGrid(new ClienteFiltro { Ordenacao = "Nome" }).ToList();
        ...
        var dtp = new DateTimePicker { Location = new Point(160, 138), Width = 220, Value = DateTime.Today.AddDays(7) };
        dlg.Controls.Add(new Label { Text = "Cliente", Location = new Point(20, 18), AutoSize = true });
        ...
        var t = new TituloReceber
        {
            ClienteId = cb.SelectedValue is long id ? id : null,
    ...
    private void BtnEditar_Click(object? s, EventArgs e)
    {
        ...
        cb.ValueMember = nameof(ClienteGridRow.Id);
        cb.DataSource = _clientes.PesquisarParaGrid(new ClienteFiltro { Ordenacao = "Nome" }).ToList();
        if (t.ClienteId.HasValue)
            cb.SelectedValue = t.ClienteId.Value;
        else
            cb.SelectedIndex = -1;
        ...
        var lblRec = new Label { Location = new Point(20, 172), AutoSize = true, Text = $"Já recebido: {t.ValorRecebido:C} (use Receber para lançar pagamentos)" };
    ...
    private void BtnReceber_Click(object? s, EventArgs e)
    {
        var id = IdSel();
        if (id is null)
        {
        ...
        if (dlg.ShowDialog(this) != DialogResult.OK)
            return;
        var valor = n.Value;
        _repo.RegistrarRecebimento(id.Value, valor);
        Carregar();
    }
    ...
    private void BtnExcluir_Click(object? s, EventArgs e)
    {
        ...
        if (id is null)
            return;
        if (_grid.CurrentRow?.DataBoundItem is TituloReceberGridRow row &&
            string.Equals(row.Situacao, "Quitado", StringComparison.OrdinalIgnoreCase))
        {
            MessageBox.Show("Não é possível excluir um título quitado.", "Contas a receber", MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }
